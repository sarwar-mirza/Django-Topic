...
def show_user_data(request):
    ...
    data2 = User.objects.filter(email='sadiya@gmail.com')
    data3 = User.objects.filter(page__page_cat='Programming')
    data4 = User.objects.filter(post__post_publish_date='2023-9-5')
    data5 = User.objects.filter(song__song_duration=6)
...
def show_page_data(request):
    ...
    data3 = Page.objects.filter(user__email='sadiya@gmail.com')
...
def show_post_data(request):
    ...
    data2 = Post.objects.filter(post_publish_date='2023-9-5')
    data3 = Post.objects.filter(user__username='Sadiya')
...
def show_song_data(request):
    ...
    data2 = Song.objects.filter(song_duration=6)
    data3 = Song.objects.filter(user__username='Sadiya')
